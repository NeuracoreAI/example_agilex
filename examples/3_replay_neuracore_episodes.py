...
if __name__ == "__main__":
    ...
    if args.episode_index == -1:
        ...
        if args.episode_index >= len(synced_dataset) or args.episode_index < 0:
            print(
                f"❌ Error: Index {args.episode_index} is out of bounds (Dataset has {len(synced_dataset)} episodes)."
            )
            print(
                "💡 Reminder: If the frontend says episode '3', you must request index '2'."
            )
            robot_controller.stop_control_loop()
            robot_controller.cleanup()
            sys.exit(1)
    ...
    try:
        for episode_idx in episode_indices:
            ...
            if not joint_positions:
                print(
                    f"⚠️ Warning: No valid joint data found in episode {episode_idx}. Skipping..."
                )
                continue
